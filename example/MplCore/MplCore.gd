...
func mint_with_guard_group():
	...
	var ix = candy_guard.new().mintV1(guard_group_pda, candy_pid, candy_machine_with_guard_group, cm_pda, payer, payer, payer, core_asset_guard_group, core_collection_guard_group, core_pid, SystemProgram.get_pid(), "Sysvar1nstructions1111111111111111111111111", "SysvarS1otHashes111111111111111111111111111", load("res://resources/new_candy_guard_data.tres").default.get_extra_account_metas(), load("res://resources/new_mint_asset_args.tres").serialize_core_mint_args(), AnchorProgram.option("base"))
	tx.add_instruction(ix)
