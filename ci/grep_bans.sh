...
patterns=(
    ...
    '(Log|SdkLog)\.(v|d|i|w|e).*(resultDictJson|errorDictJson|timeoutDictJson|cancelledDictJson|reauthDictJson)'
...
descs=(
    ...
    "Log.* or SdkLog.* call referencing an MWA NativeBridge payload variable name (Story 2-1 Task 4; Story 4-3 T2 added 'reauthDictJson'). 'resultDictJson' / 'errorDictJson' / 'timeoutDictJson' / 'cancelledDictJson' / 'reauthDictJson' are serialized MWA Dictionary payloads that include auth_token (connect path), correlation data to reconstruct a session (error/timeout/cancel paths), or developer_details with exception class names (reauth_required keystore_corrupt path). The existing log-authtoken ban (pattern 1) only catches direct 'authToken' references — payload vars wrap the token inside a JSON string where the literal 'authToken' substring is not present in the logging call arguments (the LITERAL is inside the payload at runtime). This pattern closes that gap at the payload-var name level: if it's named *DictJson and you're logging it, that's a token-leak candidate regardless of how the payload was built."
