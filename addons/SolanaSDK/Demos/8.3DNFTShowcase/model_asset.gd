...
func setup(asset:WalletAsset, auto_load_model:bool=false) -> void:
	self.asset = asset
	if auto_load_model:
		try_load_model()
...
func try_load_model() -> void:
	if asset==null:
		push_error("Couldnt find the Asset to load model of")
		return
	if asset.model_state!=null:
		instantiate_model(asset.model_state)
		return
		
	loading_template.visible=true
	
	await asset.try_load_model()
	if asset.model_state!=null:
		instantiate_model(asset.model_state)
	else:
		print("Couldn't load the model for mint: %s" % asset.mint.to_string())
...
func instantiate_model(state:GLTFState) -> void:
	...
	#print(state.json)
	model = gltf_document.generate_scene(asset.model_state,30,true,true)
	var error = convert_to_mesh_instance(model)
	if error == OK:
		add_child(model)
		scale_model_to_target()
		self.name = asset.metadata.get_token_name()
	else:
		model.queue_free()
