...
## Safely disconnects a callable from a signal if it is still connected.
...
func _wait_for_authorization(start_call: Callable) -> Dictionary:
	...
	var is_session_established := false
	...
	var established_cb = func() -> void:
		...
		if key_variant != null:
			if key_variant is PackedByteArray:
				...
				result = {"error": "unexpected key type"}
				return
		else:
			push_error("MWA: get_connected_key() returned null — cannot determine account address.")
			result = {"error": "null key"}
			return
	...
	var elapsed = 0.0
	while !is_session_established and elapsed < connect_timeout_sec:
		...
		if status == 1:
			...
			_safe_disconnect(wallet_adapter.connection_established, established_cb)
			_safe_disconnect(wallet_adapter.connection_failed, failed_cb)
			is_session_established = true
			break
		elif status == 2:
...
func _wait_for_signing(start_call: Callable) -> Dictionary:
	...
	while !is_session_established and elapsed < connect_timeout_sec:
		...
		if status == 1:
			...
			_safe_disconnect(wallet_adapter.message_signed, signed_cb)
			_safe_disconnect(wallet_adapter.signing_failed, failed_cb)
			ok = true
			is_session_established = true
			break
		elif status == 2:
...
func signMessages(message_payloads: Array) -> Dictionary:
	...
	if final_key != null:
		if final_key is PackedByteArray:
			...
			return {"error": "unexpected key type"}
	else:
		push_error("MWA: getConnectedKey() returned null — cannot determine address for signMessages.")
		signing_failed.emit("null key")
...
func _wait_for_capabilities(start_call: Callable) -> Dictionary:
	...
	while !is_session_established and elapsed < connect_timeout_sec:
		...
		if status == 1:
			...
			json_text = json
			# Disconnect one-shot listeners before manually resolving.
			_safe_disconnect(wallet_adapter.capabilities_received, received_cb)
			_safe_disconnect(wallet_adapter.capabilities_failed, failed_cb)
			is_session_established = true
			break
		elif status == 2:
