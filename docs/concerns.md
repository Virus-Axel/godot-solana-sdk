...
- **Status: RESOLVED (PENDING FIRST CI GREEN)** | **Date:** 2026-04-22 | **Resolved-by:** Story 1-5 Task 4b (Phase D) + Task 6
  - `src/mwa/tests/test_secret_string.cpp` landed in commit `f025f1c9` (T4b Phase D): two `TEST(SecretString, ...)` cases ported verbatim from Story 1-2 §Task 2. SecretString public API at `src/mwa/include/secret_string.hpp` is unchanged from the 1-2 landing, so the port compiles mechanically.
  - `scons tests` target landed in `f025f1c9` (T4b Phase C) — compiles `test_secret_string.cpp` with `-DMWA_TESTING=1` linked against `gtest-all.cc` + `gtest_main.cc` from the vendored submodule at `third_party/googletest` (v1.14.0, added `deb59965`).
  - CI wiring landed in `ce20c0a6` (T6): `.github/workflows/cpp_tests.yml` runs `scons tests && ./bin/mwa_tests` on ubuntu-latest + macos-latest.
  - **Full resolution requires first CI run to report GREEN on both tests.** Local `scons` is not installed per A-8 posture, so local empirical verification isn't possible. If CI surfaces godot-cpp host-mode linking issues (known risk flagged in T4b commit body), CR-8 flips back to OPEN until fixed. If CI is GREEN, strike "PENDING FIRST CI GREEN" and close.
