...
## A-19: DD-5-6-2 wording supersedes "6 native targets" → "7 release targets (incl. ios-release)" — `release_on_tag.yml` matches existing `build.yml` matrix; surfaced post-commit by Story 5-6 code-review

- **Story:** 5-6 | **Date:** 2026-04-30 | **Author:** Story 5-6 code-review-agent (finding #3, LOW) | **Severity:** LOW (documentation drift; zero behavioral impact)
- **Summary:** DD-5-6-2 LOCKED in `docs/stories/5-6.md:120` and the equivalent passage in `docs/architecture.md` describes the `release_on_tag.yml` matrix as "the same **6 native targets** `build.yml` covers today (linux-debug + linux-release + windows-release + macos-release + android-release-arm64 + android-release-x86_64 + web-release)." The actual `build.yml` matrix at master HEAD (lines 14-123) carries **8 release targets** — the 6 originally enumerated PLUS `ios-debug` (line 112) AND `ios-release` (line 119). Story 5-6 T11 (`release_on_tag.yml`) correctly mirrors `build.yml`'s release-only entries and includes `ios-release` (lines 106-111 of the workflow). The implementation is correct; only the LOCKED DD's count + enumeration wording is stale.
- **Root cause:** DD-5-6-2 was authored at story-creation HALT 2026-04-30 by reading the then-current `build.yml` matrix and snapshotting a count. iOS entries were already in `build.yml` at that point (per `git log .github/workflows/build.yml`). The DD captured a partial enumeration as if it were the full set. Numbers in LOCKED DDs are review-time-correct only when paired with the full enumeration; when the DD enumerates "6 native targets (linux + windows + macos + android-arm64 + android-x86_64 + web)" and the source-of-truth file has 7 release targets, the DD becomes mechanically false.
- **Decision:**
  1. **Supersede DD-5-6-2 wording in `docs/stories/5-6.md:120` and `docs/architecture.md` (wherever DD-5-6-2 is mirrored) going forward.** Original wording "the same 6 native targets `build.yml` covers today (linux-debug + linux-release + windows-release + macos-release + android-release-arm64 + android-release-x86_64 + web-release)" is amended to "the **release-only entries from `build.yml`** at HEAD — currently 7 targets (linux-release + windows-release + macos-release + android-release-arm64 + android-release-x86_64 + web-release + **ios-release**). Per A-19, the canonical reference is the source-of-truth `build.yml` matrix; the count is review-time-correct only when paired with the file's HEAD state." `release_on_tag.yml` already implements all 7 entries correctly. **stories/5-6.md and architecture.md are NOT patched** per LaiM convention (frozen Phase-3/4 artifacts); story-creator-agent + code-review-agent + Gate 5 evaluators read BOTH the original artifact and `docs/amendments.md` and use the amended value where they conflict. This amendment IS the binding diff record.
  2. **No source / workflow changes required.** `release_on_tag.yml` already mirrors `build.yml` correctly; the synthetic-tag verification path is fixed in a separate code-review fix (Finding #1 — see `git log` for the post-review fix commit).
- **Architectural posture going forward:** When a LOCKED design decision names a count of items, prefer "matches `<source-of-truth-file>` at HEAD" over a numeric enumeration. Numeric enumerations age poorly when the source file evolves. This pattern surfaces in `~/.claude/rules/retrospect/universal.md` as a code-review knowledge-gap rule (Story 5-6 finding #3).
- **Closes:** Story 5-6 code-review finding #3 (LOW). No prior CR.
- **Follow-through:**
  - At post-review fix-up commit: log A-19 in `docs/concerns.md` Story 5-6 section as a documentation-drift closure.
  - At Gate 5 evaluation: A-19 wording IS authoritative for DD-5-6-2 going forward; reviewers reading `docs/stories/5-6.md:120` MUST cross-reference `docs/amendments.md#A-19` per LaiM amendment convention.
  - Sprint-status `5-6.amendments` array updated from `[]` → `["A-19"]`.
