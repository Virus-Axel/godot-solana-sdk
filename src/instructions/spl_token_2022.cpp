...
const std::string TokenProgram2022::PID = "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb";

PackedByteArray TokenProgram2022::metadata_update_discriminator() {
	const auto DISCRIMINATOR_ARRAY = { 0xdd, 0xe9, 0x31, 0x2d, 0xb5, 0xca, 0xdc, 0xc8 };
	return Array(DISCRIMINATOR_ARRAY);
}
PackedByteArray TokenProgram2022::metadata_initialize_discriminator() {
	const auto DISCRIMINATOR_ARRAY = { 0xd2, 0xe1, 0x1e, 0xa2, 0x58, 0xb8, 0x4d, 0x8d };
	return Array(DISCRIMINATOR_ARRAY);
}
...
void TokenProgram2022::_bind_methods() {
	...
	ClassDB::bind_static_method("TokenProgram2022", D_METHOD("freeze_account", "account_pubkey", "mint_pubkey", "freeze_authority"), &TokenProgram2022::freeze_account);
...
Variant TokenProgram2022::freeze_account(const Variant &account_pubkey, const Variant &mint_pubkey, const Variant &freeze_authority) {
	const Variant new_pid = memnew_custom(Pubkey(String(PID.c_str())));
...
Variant TokenProgram2022::initialize_metadata_pointer(const Variant &payer, const Variant &mint_keypair) {
	...
	result->append_meta(*memnew_custom(AccountMeta(mint_keypair, true, true)));
...
Variant TokenProgram2022::initialize_metadata(const Variant &payer, const Variant &mint_keypair, const String &name, const String &symbol, const String &uri) {
	...
	result->append_meta(*memnew_custom(AccountMeta(mint_keypair, false, true)));
