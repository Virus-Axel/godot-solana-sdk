...
const uint32_t DEFAULT_TOKEN_DECIMALS = 9;
...
class TokenProgram : public Node {
	GDCLASS_CUSTOM(TokenProgram, Node)
private:
...
protected:
	 ...
	 */
	static Variant _initialize_mint(const Variant &token_program_pid, const Variant &mint_pubkey, const Variant &mint_authority, const Variant &freeze_authority = nullptr, uint32_t decimals = DEFAULT_TOKEN_DECIMALS);
	 ...
	 */
	static Variant _set_authority(const Variant &token_program_pid, const Variant &mint_account, const Variant &current_authority, const Variant &new_authority, uint8_t authority_type);
...
public:
	 ...
	 */
	static Variant new_delegate_record_address(const Variant &update_authority, const Variant &mint, const Variant &delegate_address, MetaDataDelegateRole role);
