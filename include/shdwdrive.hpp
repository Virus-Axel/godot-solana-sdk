...
class StorageAccountV2 : public Resource {
	GDCLASS_CUSTOM(StorageAccountV2, Resource)
private:
...
class UserInfo : public Resource {
	GDCLASS_CUSTOM(UserInfo, Resource)
private:
...
public:
	 ...
	 */
	[[nodiscard]] uint32_t get_account_counter() const;
...
private:
	...
	static String get_upload_message(const Variant &account_key, const String &filename_hash);
...
public:
	 ...
	 */
	void set_simulate_only(bool simulate_only);

	/**
	 * @getter{simulate_only, bool}
	 */
	[[nodiscard]] bool get_simulate_only() const;
